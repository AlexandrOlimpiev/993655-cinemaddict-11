...
const createCommentMarkup = (comment) => {
  ...
  const fullCommentDate = `${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()} ${date.getHours()}:${date.getMinutes()}`; // Доделать формат даты коментария в зависимости от давности комента

  return (
    `<li class="film-details__comment">
    <span class="film-details__comment-emoji">
      <img src="./images/emoji/${emojis}.png" width="55" height="55" alt="emoji-${emojis}">
    </span>
    <div>
...
const creatPopupTemplate = (film) => {
  ...
  const commentMarkup = comments.map((it) => createCommentMarkup(it));
